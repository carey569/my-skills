...
从已审批的设计方案生成 test-specs，经人审批后冻结。
...
- 检查 `designs/` 目录下是否存在已审批的设计文档（状态为「已审批」）
  - 存在 → 以设计文档为主要输入，从中提取接口定义、数据结构、核心流程、错误场景等
  - 不存在 → 检查 `.auto-dev.yaml` 的 `frozen_files` 中是否有 `designs/` 下的文件
    - 有冻结的设计文档 → 读取该文件
    - 无 → 提示用户：「未检测到已审批的设计方案。建议先执行 `/auto-dev-design` 生成设计方案。如需跳过设计直接生成 test-spec，请回复"跳过"。」
- 补充读取 `docs/` 目录下的需求文档（`.md`、`.html`）作为辅助输入
- 如果用户直接提供了需求描述，以该描述为补充输入
