...
2. **冻结边界**：用户审批过的 `test-specs/*.yaml`、`verify.sh`、Agent T 生成的测试文件标记为冻结，实现阶段不可修改。冻结文件通过 SHA-256 校验和强制保障完整性。
3. **自愈循环上限**：单元测试最多重试 5 轮，集成测试最多 3 轮，超限暂停并向用户报告失败原因。
4. **先理解再动手**：修改任何代码前，必须先读取相关文件理解上下文。
5. **错误脱敏**：面向用户的错误信息不得暴露内部路径、堆栈或敏感配置。
6. **进度可恢复**：所有阶段性进展记录到 `.progress/status.md`，支持中断后恢复。

### 冻结文件校验

在以下时机校验冻结文件的 SHA-256 校验和：
- Phase C 开始前（`/auto-dev-run` 入口）
- Phase C 完成后（生成报告前）
- 每次自愈循环开始时（如果已完成超过 2 轮）

校验方法：对 `.auto-dev.yaml` 中 `frozen_files` 列表的每个文件计算 SHA-256，与 `frozen_checksums` 中的记录比对。不一致则立即暂停。
