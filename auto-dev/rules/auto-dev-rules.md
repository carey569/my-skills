...
2. **设计先行**：L/XL 任务必须先生成设计方案，经人审批后才能生成 test-spec
3. **测试与实现分离**：L/XL 任务使用独立 Agent 分别负责测试和实现
4. **冻结边界**：用户审批过的文件（设计文档、test-specs、verify.sh）通过 SHA-256 校验和强制保障完整性
5. **自愈循环上限**：单元测试最多 5 轮，集成测试最多 3 轮
6. **错误脱敏**：面向用户的错误信息不得暴露内部路径、堆栈或敏感配置
7. **进度可恢复**：所有阶段性进展记录到 `.progress/status.md`
